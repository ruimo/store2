...
@Singleton
class ItemPriceStrategyRepo @Inject() (
  cache: Cache
) {
  ...
  val ItemPriceStrategyConf: () => immutable.Map[UserTypeCode, ItemPriceStrategy] = 
    cache.config { cfg =>
      val strategyCfg = cfg.getOptional[Configuration]("itemPriceStrategy").getOrElse(
      ...
      classOf[UserTypeCode].getEnumConstants().map { utc =>
        val clsName =
          strategyCfg.getOptional[String](utc.toString + ".type").getOrElse("models.UnitPriceStrategy")
