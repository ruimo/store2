...
RUN cd /opt/functional-store2 && \
  ...
  echo /opt/functional-store2/$cmd/bin/functional-store2 -J-Xmx2048m -DmoduleName=$cmd -DapplyEvolutions.default=true -Dplay.crypto.secret=\${APP_SECRET} >> launch.sh && \
  chmod +x launch.sh
